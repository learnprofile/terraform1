...
az iot hub create --name "$IOT_NAME" --resource-group "$RG_NAME" --sku S1 --partition-count 4

az iot hub device-identity create --device-id "$DEVICE_ID" --hub-name "$IOT_NAME" --resource-group "$RG_NAME"

az storage account create --name "$STORAGE_NAME" --resource-group "$RG_NAME" --kind StorageV2 --sku Standard_LRS --access-tier Cool
